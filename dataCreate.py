...
class dataCreate:
    ...
    def __init__(self, numRob, border, maxVel, dt, devInput, devObser):
        self.velocity = np.zeros((3, numRob)) # x_dot, y_dot, theta_dot
        self.numRob = numRob
        self.border = border
        self.maxVel = maxVel
        self.dt = dt
    ...
    # such that all robots fly randomly within the border and avoid each other
        if (step % 500 == 0):
            ...
            self.velocity[0:2, :] = np.random.uniform(0, self.maxVel*2, (2, self.numRob)) - self.maxVel # numpy.random.uniform(low,high,size)
        ...
        for i in range(self.numRob):
            ...
            for j in range(self.numRob):
                if j!= i:
                    ...
                    if distIJ < 1:
                        # simple and effective ways to avoid collision
